...
We've isolated that one html table and turned it into a dataframe, albeit one that is nested in a list.  Let's extract it from the list. 
...
https://github.com/fivethirtyeight/data/blob/4c1ff5e3aef1816ae04af63218015066e186c147/avengers/avengers.csv

https://raw.githubusercontent.com/fivethirtyeight/data/4c1ff5e3aef1816ae04af63218015066e186c147/avengers/avengers.csv
...
* DUPLICATE ROWS - There are two apparently identical rows for Emma and Amelia. We should really only have one, and could lead to misleading summarization if we kept both.
...
Now that we've completely cleaned up our data, we might want to export. Here's how to export to csv.

```{r}
write.csv(babynames_clean, "data-clean/babynames_clean.csv", row.names = FALSE)
```
